...
async function cancelTransaksi(request, env) {
  ...
  const stokSebelum = Number(trx.stok_lama ?? trx.stok_awal ?? bahan?.stok ?? 0);
  await env.DB.batch([
    env.DB.prepare("UPDATE bahan SET stok=? WHERE sku=?").bind(stokSebelum,trx.sku),
    env.DB.prepare("UPDATE transaksi SET keterangan=keterangan || ' [DIBATALKAN]' WHERE id_transaksi=?").bind(text(idTransaksi))
  ]);
  return json({ success:true, message:"Transaksi dibatalkan", stokTerakhir:stokSebelum });
}
...
async function renderReport(url, env) {
  ...
  </style></head><body><div class="toolbar"><button onclick="window.print()">Cetak / Simpan PDF</button><button onclick="window.close()">Tutup</button></div><main class="page"><img class="banner" src="/report-header.png" alt="Boss Inventory Report"><h1 class="title">${esc(title)}</h1><div class="meta">Periode: ${esc(range.label)} &nbsp; | &nbsp; Dibuat: ${esc(generated.toLocaleString("id-ID", { timeZone: "Asia/Jakarta" }))}<br>Kode Laporan: ${esc(reportId)}</div>${sections}<div class="sign"><div class="signline"></div>Petugas / Penanggung Jawab</div><div class="footer">${esc(reportId)} • Laporan Gudang Roti Boss</div></main></body></html>`;
