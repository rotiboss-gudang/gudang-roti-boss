...
function reportSummary(bahan, low, expired) { return `<section class="summary"><div class="card"><b>${bahan.length}</b><span>Total Jenis Bahan</span></div><div class="card"><b>${low.length}</b><span>Stok Menipis</span></div><div class="card"><b>${expired.length}</b><span>Expired / Segera</span></div><div class="card"><b>${bahan.reduce((sum, b) => sum + (Number(b.stok) || 0), 0).toFixed(2)}</b><span>Total Stok</span></div></section>`; }
